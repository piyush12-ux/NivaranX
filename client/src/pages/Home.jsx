  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <motion.nav
        ...
        className="flex justify-between items-center px-4 md:px-8 py-4 border-b border-gray-800"
      >
        ...
        <div className="hidden md:flex gap-4">
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onAbout}
            className="px-6 py-2 text-gray-300 rounded-xl font-bold hover:text-white">
            About
          </motion.button>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onContact}
            className="px-6 py-2 text-gray-300 rounded-xl font-bold hover:text-white">
            Contact
          </motion.button>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onLogin}
            className="px-6 py-2 border border-blue-500 text-blue-400 rounded-xl font-bold hover:bg-blue-500/10">
            Login
          </motion.button>
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onRegister}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="md:hidden bg-gray-900 border-b border-gray-800 px-4 py-4 flex flex-col gap-3"
          >
            <button onClick={() => { onAbout(); setMenuOpen(false) }}
              className="text-gray-300 py-2 text-left font-bold">About</button>
            <button onClick={() => { onContact(); setMenuOpen(false) }}
              className="text-gray-300 py-2 text-left font-bold">Contact</button>
            <button onClick={() => { onLogin(); setMenuOpen(false) }}
              className="w-full py-3 border border-blue-500 text-blue-400 rounded-xl font-bold">
              Login
            </button>
            <button onClick={() => { onRegister(); setMenuOpen(false) }}
              className="w-full py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl font-bold">
              Register
            </button>
          </motion.div>
        )}
      ...
      {/* Hero Section */}
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-12 md:py-20 text-center">
        <motion.div
        ...
        <motion.div
          ...
          className="grid grid-cols-3 gap-3 md:gap-6 mt-12 md:mt-20"
        >
          <div className="bg-gray-900 border border-gray-800 p-4 md:p-6 rounded-2xl">
            <p className="text-2xl md:text-4xl font-bold text-blue-400">500+</p>
            <p className="text-gray-400 mt-2 text-xs md:text-base">Complaints Resolved</p>
          </div>
          <div className="bg-gray-900 border border-gray-800 p-4 md:p-6 rounded-2xl">
            <p className="text-2xl md:text-4xl font-bold text-green-400">98%</p>
            <p className="text-gray-400 mt-2 text-xs md:text-base">Success Rate</p>
          </div>
          <div className="bg-gray-900 border border-gray-800 p-4 md:p-6 rounded-2xl">
            <p className="text-2xl md:text-4xl font-bold text-purple-400">24hrs</p>
            <p className="text-gray-400 mt-2 text-xs md:text-base">Avg Resolution Time</p>
          </div>
        </motion.div>
        ...
        >
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-8">Why Choose NivaranX?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <motion.div
                ...
                className="bg-gray-900 border border-gray-800 p-5 rounded-2xl text-left hover:border-blue-500/50 transition-all"
              >
                <span className="text-3xl">{feature.icon}</span>
                <h3 className="text-white font-bold text-base mt-3">{feature.title}</h3>
        ...
        <motion.div
          ...
          className="mt-12 md:mt-20"
        >
          <h2 className="text-2xl md:text-3xl font-bold text-white mb-8">Complaint Categories</h2>
          <div className="flex flex-wrap gap-3 justify-center">
            {['🛣️ Road Damage', '💧 Water Supply', '⚡ Electricity', '🗑️ Garbage Collection', '💡 Street Light', '🚨 Public Safety'].map((cat, index) => (
              <motion.span
                key={index}
                whileHover={{ scale: 1.1 }}
                className="bg-gray-900 border border-gray-700 px-4 py-2 rounded-full text-gray-300 font-medium hover:border-blue-500/50 cursor-pointer text-sm"
              >
                {cat}
              </motion.span>
            ))}
        ...
        <motion.div
          ...
          className="mt-12 md:mt-20 border-t border-gray-800 pt-8 text-gray-500 text-sm flex flex-col md:flex-row justify-center gap-4 md:gap-8"
        >
